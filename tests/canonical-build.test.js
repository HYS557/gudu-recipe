...
test('canonical build is deterministic, preserves 961 existing IDs and adds six categories', () => {
  ...
  assert.equal(a.recipes.length, 1482);
  assert.equal(new Set(a.recipes.map(recipe => recipe.id)).size, 1482);
  assert.equal(a.recipes.filter(recipe => !recipe.id.startsWith('rec_foundation_')).length, 961);
