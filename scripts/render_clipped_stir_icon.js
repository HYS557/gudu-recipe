...
// =============================================================================
// 彻底解决勺子出界问题：
// 1. 语法严谨（修正 roundRect 参数）
// 2. 勺柄自猫猫双爪倾斜入锅，在锅沿厚唇之下！
// 3. 严格使用 ctx.clip() 限制在锅口椭圆区域，勺头在汤水深处，绝不跨出锅唇半步！
// 4. 水纹是贴合汤面透视的紧凑椭圆涟漪，绝无巨大圆形气泡罩！
// =============================================================================
const scriptClippedStir = `
  ...
  // 3. Cat Mascot: Head, Ears, Hat & Cute Expression (Background Layer)
  ctx.save();
  ...
  // ---------------------------------------------------------------------------
  // 6. 木勺柄上截（位于猫猫双手间，向下探入锅中）
  // ---------------------------------------------------------------------------
  ctx.save();
  ctx.translate(510, 480);
  ctx.rotate(0.18);
  roundRect(ctx, -11, -50, 22, 115, 11);
  const handleGrad = ctx.createLinearGradient(0, -50, 0, 65);
  handleGrad.addColorStop(0, '#F7B06D');
  handleGrad.addColorStop(1, '#CF7530');
  ...
  ctx.rotate(0.18);
  // 勺柄下半截（深入汤底）
  roundRect(ctx, -11, 45, 22, 55, 11);
  ctx.fillStyle = '#B55A1A'; ctx.fill();
  ctx.strokeStyle = lineCol; ctx.lineWidth = 8; ctx.stroke();

  // 木勺圆弧勺头（位于汤心）
  ctx.beginPath();
  ctx.ellipse(0, 95, 36, 24, 0, 0, Math.PI * 2);
  ctx.fillStyle = '#9C4A11'; ctx.fill();
  ctx.strokeStyle = lineCol; ctx.lineWidth = 8; ctx.stroke();
  ctx.restore();

  // (D) 覆盖在勺头上的金色半透明汤水（表明勺在汤下，真实浸没）
  const stirWaterCover = ctx.createRadialGradient(530, 575, 5, 530, 575, 55);
  stirWaterCover.addColorStop(0, 'rgba(255, 172, 0, 0.75)');
  stirWaterCover.addColorStop(0.7, 'rgba(240, 130, 0, 0.5)');
  ...
  // (E) 贴合透视的扁平椭圆水面涟漪（紧凑限定在汤面内，决无大圆球！）
  ...
  // 8. Top Glass Reflection on App Icon
  ctx.save();
  roundRect(ctx, 32, 32, 960, 960, 220);
  ctx.clip();
...
const htmlContent = createHtml(scriptClippedStir);
const tempHtmlPath = path.join(tempDir, 'clipped_stir.html');
const tempPngPath = path.join(tempDir, 'clipped_stir.png');
...
console.log('Rendering strictly clipped in-pot stirring icon...');
...
console.log('Strictly in-pot stirring icon generated successfully!');
